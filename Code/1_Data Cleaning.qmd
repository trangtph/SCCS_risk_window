...
cdm_eligible[c("myo", "pe", "thrc")] <- lapply(cdm_eligible[c("myo", "pe", "thrc")], function(df) {
  df %>%
    dplyr::left_join(cdm_eligible$individual %>% select(id, obs_sta_date, obs_end_date), by = "id") %>%
...
all_ids <- unique(unlist(lapply(cdm_eligible, `[[`, "id")))

cdm_eligible <- lapply(cdm_eligible, function(df) {
