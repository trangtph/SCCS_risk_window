...
surv_obj <- with(cdm_sccs$myo_sccs,
                 Surv(time = obs_end, event = rep(1, nrow(cdm_sccs$myo_sccs))))
...
plot(
  ...
  ylab = "Individuals remaining under observation (%)",
...
test_event_dep_obs <- function(dat) {
  ...
  list(
    models = list(
      base = m_base$coefficients,
      dep_obs_bnt = m_dep1$coefficients,
      dep_obs_chad = m_dep2$coefficients
    ),
    lrt = list(
