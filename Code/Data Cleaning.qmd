...
new_thrc <- data.frame(ID = sample(individual$ID, size = 50, replace = FALSE),
                  HOSP_ID = rep(NA_character_, 50),
                  primary_diagnosis = as.integer(rep(1, 50)),
                  diagnosis_date = as.Date(sample(seq(as.Date("2020-01-01"),
                                                      as.Date("2022-05-31"), 
                                                      by = "day"), 
                                                  size = 50, replace = TRUE)),
                  diagnosis_code = sample(c("M311", "D693", "D694"), size = 50, replace = TRUE),
                  coding_system = rep("ICD10CM", 50),
                  hosp_id_new = rep(NA_character_, 50),
                  id_new = rep(NA_character_, 50)
    )

outcome_event2 <- rbind(outcome_event, new_thrc)
export(outcome_event2, here("outcome_event2.csv"))
outcome_event <- outcome_event2
```
...
convert_date <- function(df, 
                         date_pattern = "_date(_\\d+)?$", #"_date" optionally followed by "_" and one or more digits
                        orders = c("dmy", "ymd")
                        ) {
  
  date_cols <- grep(date_pattern, names(df), value = TRUE) #collect columns whose names containing "_date"
...
-   Some sanity check: Any ICDs not classified in outcome and comorb tables?
...
```{r}
# Individuals with outcomes in the observation period
...
cdm_eligible$individual <- cdm_eligible$individual %>%
  mutate(obs_sta_date = as.Date("2020-10-01"),
         obs_end_date = pmin(admin_end, death_date, deregister_date, vacc_date_1, vacc_date_3, infect_date, na.rm = TRUE))
```
